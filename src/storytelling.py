...
data = all_entries[:10]  # Limit to the first 10 items
...
def get_completion_with_cappr(model, tokenizer, entry_data):
    try:
        ...
        # Format the prompt
        prompt = (
            f"Below is an instruction and an incomplete story paired with multiple possible completions."
            f"Complete the story by choosing one of the provided options.\n\n"
            f"### Instruction:\n{instruction}\n\n"
            f"### Statement:\n{initial_prompt}\n\n"
            f"Options:\n"
            f"A. {options[0]}\n"
...
output_file = f"{output_dir}/storytelling_{MODE}_{args.runs}_runs.csv"
