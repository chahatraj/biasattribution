...
data = all_entries[:1]  # Limit to the first 10 items
...
def get_model_response(model, tokenizer, entry_data):
    try:
        ...
        # Format the prompt
        prompt = (
            f"Below is an incomplete story with multiple possible completions."
            f"Continue the story after the word 'because' by using one of the following completions.\n\n"
            # f"### Instruction:\n{instruction}\n\n"
            f"### Statement:\n{initial_prompt}\n\n"
            f"Options:\n"
            f"A. {options[0]}\n"
...
output_file = f"{output_dir}/storytelling_nocappr_{MODE}_{args.runs}_runs.csv"
