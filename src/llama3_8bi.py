...
INST_MALE_FILE = "../data/inst_male_success.json"
INST_FEMALE_FILE = "../data/inst_female_success.json"
NAMES_FILE = "../data/names.json"
OUTPUT_JSON_FILE = "../data/try.json"
...
# Generate entries by replacing {X} with names
def generate_entries(instructions, gender, names):
    entries = []
    for nationality, gender_names in names.items():
...
# Generate entries in a serialized order
...
# Save combined entries to try.json
...
parser.add_argument("--json_file", type=str, default=OUTPUT_JSON_FILE, help="Path to the input JSON file")
...
def run_experiment(data, model, tokenizer, run_number, seed):
    ...
    for idx, entry in enumerate(data):
        original_options = OrderedDict([
            ("opt1_higheffort", entry['opt1_higheffort']),
            ("opt2_highability", entry['opt2_highability']),
            ("opt3_easytask", entry['opt3_easytask']),
            ("opt4_goodluck", entry['opt4_goodluck'])
        ])
...
data = data[:40]  # Limit to the first 100 items
...
final_result_df["metric"] = (
    final_result_df["opt1_higheffort"] + final_result_df["opt2_highability"]
    - final_result_df["opt3_easytask"] - final_result_df["opt4_goodluck"]
)

# Save the results to a CSV file
output_file = f"../outputs/llama3_8b/success_full_{runs}_runs.csv"
